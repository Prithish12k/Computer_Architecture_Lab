		...
		if (IF_OF_Latch.isOF_enable()) {
			...
			switch (opInst) {
				...
				case srl:
					...
					// Setting values
					rs1.setValue(Integer.parseInt(binInst.substring(5, 10), 2)); // set the value of register i.e. x0
																					// === 00000 (register number and
																					// not value)
			...
			if (inst.getSourceOperand1() != null && inst.getSourceOperand1().getOperandType() == OperandType.Register) {
				src1 = inst.getSourceOperand1().getValue();
			}

			if (inst.getSourceOperand2() != null && inst.getSourceOperand2().getOperandType() == OperandType.Register) {
				src2 = inst.getSourceOperand2().getValue();
			}

			boolean hazard = false;

			if (src1 >= 0 && containingProcessor.getRegisterFile().isRegisterBusy(src1))
				hazard = true;
			if (src2 >= 0 && containingProcessor.getRegisterFile().isRegisterBusy(src2))
				hazard = true;
			...
			if (hazard) {
				...
				// Only mark busy for instructions that actually write to a register
				// store, jmp, beq, bne, blt, bgt, end do NOT write a destination register
				if (opInst != OperationType.store && opInst != OperationType.jmp
						&& opInst != OperationType.beq && opInst != OperationType.bne
						&& opInst != OperationType.blt && opInst != OperationType.bgt
						&& opInst != OperationType.end) {
					int destReg = -1;

					if (inst.getDestinationOperand() != null
							&& inst.getDestinationOperand().getOperandType() == OperandType.Register) {
						destReg = inst.getDestinationOperand().getValue();
					}
